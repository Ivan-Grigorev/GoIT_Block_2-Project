...
from Test_asbot.settings import NEWS_API_KEY, WEATHER_API_KEY
...
class AddressBookView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        if self.request.GET.get('search_area'):
            context['contacts'] = context['contacts'].filter(Q(name__icontains=self.request.GET.get('search_area')) |
                                                             Q(surname__icontains=self.request.GET.get('search_area')) |
                                                             Q(phone__icontains=self.request.GET.get('search_area')))
            return context

        if self.request.GET.get('birthday_search'):
            context['contacts'] = AddressBook.objects.filter(birthday__month__gte=self.request.GET.get(
                                                                'date_from').split('-')[1],
                                                             birthday__month__lte=self.request.GET.get(
                                                                'date_to').split('-')[1])
            return context
...
def show_world_news(request):
    ...
    weather_url = urllib.request.urlopen(
        ...
        f"data/2.5/weather?q={urllib.parse.quote(user_location['city'])}&appid={WEATHER_API_KEY}"
    ).read()
...
def show_weather(request):
    ...
    if request.method == 'POST':
        weather_url = urllib.request.urlopen(
            f"https://api.openweathermap.org/"
            f"data/2.5/weather?q={urllib.parse.quote(request.POST['location'])}&appid={WEATHER_API_KEY}"
        ).read()
        weather_data = json.loads(weather_url)
        data = {
            'name': weather_data['name'],
